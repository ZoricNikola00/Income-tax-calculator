	...
	return (
		<div className="p-4 w-full text-center">
			<h1 className="text-primary text-lg my-2 font-semibold">
				Your Net {formData.freq} income is{' '}
				<br className="block md:hidden my-2" />
			...
			<div className="w-full text-primary p-2 text-md font-semibold border-primary border-2 rounded-lg mt-10">
				...
				{frequencyData.map((item) => {
					...
					return (
						<div
							key={item.id}
							className="hover:bg-primary hover:text-white transition duration-300 py-2 my-2 w-full grid grid-cols-3 text-lg border-b-2 border-secondary"
						>
							<span className="col-span-1">{item.frequency}</span>
							<span className="col-span-1">${Math.round(income)}</span>
